...
# --- 1. 頁面基礎設定 ---
st.set_page_config(page_title="2026 台灣 FIA 旗艦定價模型 (V13.1)", layout="wide")
...
# ==========================================
    ...
    with st.expander("📚 定價邏輯揭密 (Pricing Equation) - 點擊展開", expanded=True):
        ...
        st.latex(r'''
        Budget = PR \times ( \underbrace{Call_{Buy}}_{買入成本} - \underbrace{Call_{Sell}}_{賣出Cap收入} )
        ''')
        
        st.markdown("""
        * **Budget (預算):** 來自債券利息 ({:.2%})。
        * **Call_Buy:** 為了跟漲，我們必須買入 ATM Call。
        * **Call_Sell:** 為了省錢，我們必須賣出 OTM Call (設定 Cap)。
        * **PR (參與率):** 預算能買幾組這樣的組合。
        """.format(option_budget_pct))
    ...
    <b>驗算檢查：</b><br>
    Buy Call Cost (${eq_cost_long:.2f}) - Sell Cap Revenue (${eq_rev_short:.2f}) = Net Unit Cost (${eq_cost_long - eq_rev_short:.2f})<br>
    PR ({final_pr:.2f}) * Net Unit Cost = <b>${final_pr * (eq_cost_long - eq_rev_short):.2f}</b> (應接近預算 ${option_budget_amt:.2f})
    </div>
    """, unsafe_allow_html=True)
...
# ==========================================
    ...
    # Option 2 (Comparison Mode: PR=100%)
    vol_adjusted = sigma + vol_skew
    ...
    # --- Part 2: ICS 2.0 資本結構拆解 ---
    st.markdown("---")
    st.header("2. ICS 2.0 資本結構拆解 (Capital Structure)")
    st.caption("將上圖綠色的「公司總利潤」進一步拆解為風險資本成本與淨利。")
    ...
    fig_ics.add_trace(go.Bar(y=['ICS 2.0 結構'], x=[val_risk_margin*100], name='風險邊際 (Risk Margin & CoC)', orientation='h', 
                             marker=dict(color='#F39C12'), text=[f"{val_risk_margin:.2%}"], textposition='auto'))
    fig_ics.add_trace(go.Bar(y=['ICS 2.0 結構'], x=[val_net_profit*100], name='股東淨利 (Net Profit)', orientation='h', 
                             marker=dict(color='#27AE60'), text=[f"{val_net_profit:.2%}"], textposition='auto'))
    
    fig_ics.update_layout(barmode='stack', title="Issuer Spread 深度拆解 (Post-ICS 2.0)", xaxis_title="佔本金百分比 (%)", height=180, margin=dict(l=20, r=20, t=40, b=20))
    st.plotly_chart(fig_ics, use_container_width=True)

    st.info(f"""
    **ICS 2.0 觀點：**
    * **Risk Margin (40%):** 為了承擔市場風險與長壽風險，保險公司必須計提的資本成本。
    * **Net Profit (60%):** 扣除資本成本後，這才是真正的股東權益報酬 (ROE) 來源。
    """)

    # --- Part 3: 效益整合 ---
    st.markdown("---")
    st.header("3. 效益整合分析")
    market_moves = np.linspace(0.0, 0.30, 300)
    y_o1 = market_moves * pr_opt1
    y_o2 = [min(m * final_pr_o2, final_cap_o2) for m in market_moves]
    
    fig_comp = go.Figure()
    fig_comp.add_trace(go.Scatter(x=market_moves*100, y=y_o1*100, name=f'方案一 (PR={pr_opt1:.0%})', line=dict(color='#2E86C1', width=3)))
    fig_comp.add_trace(go.Scatter(x=market_moves*100, y=np.array(y_o2)*100, name=f'方案二 (PR=100% / Cap={final_cap_o2:.1%})', line=dict(color='#C0392B', width=4)))
    
    if final_pr_o2 > pr_opt1:
        cp = final_cap_o2 / pr_opt1
        if cp < 0.3:
             fig_comp.add_annotation(x=cp*100, y=final_cap_o2*100, text=f"黃金交叉: {cp:.1%}", showarrow=True, arrowhead=2, ax=40, ay=-40)

    fig_comp.update_layout(title="客戶收益比較", xaxis_title="指數漲幅 (%)", yaxis_title="客戶收益 (%)", template="plotly_white", height=500)
